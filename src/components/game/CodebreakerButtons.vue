<template>
  <div class="codebreaker-ui footer" :class="turn">
    <div @click="showRoom" class="codebreaker-ui__room">{{t('Room')}}:
    ...
    <template v-if="code.word.length > 0">
      <span class="codebreaker-ui__selection" v-if="selectedCards<code.count">
        ...
        <template v-if="code.count - selectedCards > 1">{{t('more cards')}}</template>
        <template v-else>{{t('more card')}}</template>
      </span>
      <button class="codebreaker-ui__selection selection__button" v-else @click="commitSelection()">{{t('Send selection')}}</button>
    </template>
  </div>
</template>
